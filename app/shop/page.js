...
export default function Shop() {
 ...
 return { ...product, quantity: Math.max(0, product.quantity - purchaseMap[product.id]) };
 }
 return product;
 });
 setProducts(updatedProducts);
 const updates = updatedProducts
 .filter((p) => purchaseMap[p.id])
 .map((p) => ({ id: p.id, quantity: p.quantity }));
 try {
 await fetch("/api/braze-catalog", {
 method: "PATCH",
 headers: { "Content-Type": "application/json" },
 body: JSON.stringify({ updates }),
 });
 } catch (err) {
 console.error("Failed to update Braze catalog quantities:", err);
 }
 ...
 {/* ─── Trust Badges (Sticky) + Cart Button ─── */}
 <div className="bg-white border-b sticky top-0 z-50">
 <div className="w-full px-8 py-4 grid grid-cols-3 gap-4 relative">
 {[
 ...
 <Button
 variant="outline"
 className="absolute right-3 top-1/2 -translate-y-1/2 flex flex-col items-center gap-1 px-5 py-4 rounded-2xl bg-gray-900 text-white shadow-xl hover:bg-gray-800 cursor-pointer border border-gray-700"
 >
 ...
 {/* ─── Cart Drawer ─── */}
 {/* ✅ FIX: [&>button.absolute]:hidden hides ONLY the default Shadcn X, not our custom one */}
 <SheetContent className="w-[420px] flex flex-col p-0 bg-white [&>button.absolute]:hidden">
 ...
 {/* ✅ Our custom X — not affected by the selector above */}
 <button
 onClick={() => setCartOpen(false)}
 className="w-9 h-9 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 transition cursor-pointer text-gray-600 hover:text-gray-900"
 >
 <X className="w-5 h-5" />
 </button>
 </div>
 ...
 <div
 key={index}
 className="flex items-center gap-4 p-4 rounded-2xl bg-gray-50 border border-gray-100 hover:border-gray-200 transition"
 >
 <img
 ...
 className="w-full py-4 rounded-2xl text-base font-bold cursor-pointer bg-black hover:bg-gray-800 disabled:bg-gray-200 disabled:cursor-not-allowed"
 >
 ...
 className="pl-10 rounded-xl border-gray-200 focus:border-black focus:ring-black cursor-text"
 />
 </div>
 ...
 {/* ✅ FIX: Show "Out of Stock" badge when 0, "Only X left!" when <= 5 */}
 {p.quantity === 0 ? (
 ...
 <Badge className="bg-yellow-200 text-gray-600 border-0 text-xs font-semibold px-3 py-1">
 {p.category}
 </Badge>
 </div>
 <Button
 ...
 : addedMap[p.id]
 ? "bg-green-500 hover:bg-green-500 scale-95 cursor-pointer"
 : "bg-black hover:bg-gray-800 cursor-pointer"
 }`}
 >
 {p.quantity === 0 ? "Out of Stock" : addedMap[p.id] ? "✓ Added to Cart!" : "Add to Cart"}
 </Button>
 </div>
 </div>
 ))}
 ...
 {selectedProduct.quantity === 0 ? "Out of Stock" : `${selectedProduct.quantity} units`}
 </p>
 </div>
 </div>
 <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-2xl">
 ...
 className="flex items-center gap-2 text-blue-500 hover:text-blue-700 text-sm font-medium hover:underline cursor-pointer"
 >
 ...
 className="flex-1 rounded-2xl py-4 text-base font-bold cursor-pointer bg-black hover:bg-gray-800 disabled:bg-gray-200 disabled:cursor-not-allowed"
 >
